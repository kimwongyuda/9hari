...
class Pagination extends Component{
    ...
    render(){
        ...
        const posts_show = posts_list.map((item, index) => (
            ...
            <Link onClick={() => this.warpfunc(inputcount*(maxpage-pageint) + (posts_list.length-index))} to ={`/suggest/${cur_page_index}/${pageint}/${inputcount*(maxpage-pageint) + (posts_list.length-index)}`} className={style.tablelink}><span className={style.smallbox3} style={{textAlign: 'left', width: '35%'}}>{item.title}</span></Link>
